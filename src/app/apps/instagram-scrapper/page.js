...
const Page = () => {
  ...
  useEffect(() => {
    ...
    // 🚫 If Instagram not found in profile
    if (!user.instagram) {
      setStatus("danger");
      setData(
        <div className="d-flex align-items-center justify-content-between flex-wrap">
          <div className="d-flex align-items-center">
            ...
            <span>
              Please add your <strong>Instagram</strong> or{" "}
    ...
    // ✅ Instagram exists — run API
    const fetchData = async () => {
      setLoading(true);
      setStatus("info");
      setData("Fetching Instagram data…");

      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/n8n/instagram/public`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            credentials: "include",
            body: JSON.stringify({ instagram: user.instagram }),
          }
        );
    ...
    <DefaultLayout>
      <div className="container py-4">
        <h3 className="mb-4">
          Instagram Insights{" "}
          {user?.instagram && (
            <span className="text-primary">@{user.instagram}</span>
          )}
        </h3>
